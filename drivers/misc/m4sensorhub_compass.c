...
struct m4com_driver_data {
	...
	struct delayed_work         m4com_work;
...
static void m4com_work_func(struct work_struct *work)
{
	int err = 0;
	struct m4com_driver_data *dd = container_of(work,
						    struct m4com_driver_data,
						    m4com_work.work);
	int size = 0;
	struct timespec ts;
	...
	if (dd->samplerate > 0)
		queue_delayed_work(system_freezable_wq, &(dd->m4com_work),
...
static int m4com_set_samplerate(struct m4com_driver_data *dd, int16_t rate)
{
	...
	cancel_delayed_work(&(dd->m4com_work));
	dd->samplerate = rate;
	if (dd->samplerate > 0)
		queue_delayed_work(system_freezable_wq, &(dd->m4com_work),
...
static int m4com_driver_init(struct init_calldata *p_arg)
{
	...
	INIT_DELAYED_WORK(&(dd->m4com_work), m4com_work_func);
...
static int m4com_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct m4com_driver_data *dd = platform_get_drvdata(pdev);
	if (m4com_set_samplerate(dd, dd->latest_samplerate) < 0)
		m4com_err("%s: setrate retry failed\n", __func__);
	return 0;
}
