...
static int m4sgm_driver_init(struct init_calldata *p_arg)
{
	...
	err = m4sensorhub_irq_register(dd->m4,
		M4SH_IRQ_SIGNIFICANT_MOTION, m4sgm_isr, iio, 1);
	if (err < 0) {
	...
	err = m4sensorhub_irq_enable(dd->m4, M4SH_IRQ_SIGNIFICANT_MOTION);
	if (err < 0) {
...
static int m4sgm_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct iio_dev *iio = platform_get_drvdata(pdev);
	struct m4sgm_driver_data *dd = iio_priv(iio);
	mutex_lock(&(dd->mutex));
	if (m4sgm_set_samplerate(iio, dd->latest_samplerate) < 0)
		m4sgm_err("%s: setrate retry failed\n", __func__);
	mutex_unlock(&(dd->mutex));
	return 0;
}
